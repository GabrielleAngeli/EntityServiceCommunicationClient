...
class MainActivity : AppCompatActivity() {
    ...
    private val incrementBoundServiceConnection = object: ServiceConnection {
        ...
        override fun onServiceDisconnected(name: ComponentName?) {
            Log.v(getString(R.string.app_name), "Client unbound from the service.")
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        incrementBoundServiceIntent = Intent().apply {
            component = ComponentName("br.edu.ifsp.scl.sdm.entityservicecommunication", "br.edu.ifsp.scl.sdm.entityservicecommunication.IncrementBoundService")
        }

        if(!bindService(incrementBoundServiceIntent, incrementBoundServiceConnection, BIND_AUTO_CREATE)){
            Log.e(getString(R.string.app_name), "Service unavailable.")
            finish()
        }
